...
export default {
  ...
  durationTicks: 6000, // 【占位预填】星域持续时间（tick）；20tps ⇒ 3 分钟
  seed: null, // 可选默认种子（null＝由星域配置界面随机生成/手输；**种子只在界面输入**）

  /* ★ M3d 迭代 2：**单船费用**（＝**每派出一艘单位收一次**：派遣 n 艘 ⇒ 扣 `n ×` 本费用；
   *   ★ **损毁不返还**：费用只在「派出/激活」这一刻收，单位损毁**不退**）。
   *   · 与「激活星域」的激活费**分开计**：激活另收固定 `activateUnits ×` 本费用（见下 `activateUnits`）；
   *   · 键＝基地资源键（`data/resources.js` 的 `energy/ore/alloy/rare/science`，**只写 > 0 的项**）；
   *   · 未知键 / 负值 ⇒ 由 `data/starfieldData.js` 的 `validateStarfieldConfig` 拦下（见 `deployCost` 校验）；
   *   · 逐档递增（H1 < H2 < H3）；数值**【占位预填 · 待用户调校】**。
   *   · ★★ M3d 迭代 4 用户已定（**方案 A**）：本值（以及激活费 / 超出加价）**可能超过**基地现有
   *     `initialResourceCaps`，这属**既有占位状态** —— **本轮不动任何数值**，**等 M4 星球
   *     `effect.resourceCap` 抬上限**（届时只改那个字段；本文件不改）。 */
  deployCost: { energy: 200, ore: 100 },

  /* ★ M3d 迭代 2：**激活星域的激活费**（以「单船费用」为单位的**倍数**）：
   *   · 激活星域（创建本档星域）一次性收取 **激活费** ＝ `activateUnits × deployCost`（**固定部分**）；
   *     ★★ M3d 迭代 3：**随行单位另按同一费率函数收派遣费** ⇒ 界面把两者**合并成一个总价**显示
   *     （用户口径："合并求和显示，不单列"；见 `systems/expedition.js` `dispatchPriceOf` + `sumCosts`）；
   *   · **允许零单位激活**（空星域先存在，随后再「派遣」逐艘注入；零单位 ⇒ 只收激活费）；
   *   · **激活不占出战名额**（★ 迭代 3：出战上限**不再拦截**任何动作，只是**费率分界**）；
   *   · 数值**【占位预填 · 待用户调校】**；缺省 ⇒ 引擎兜底常量 `DEFAULT_ACTIVATE_UNITS`（2）。 */
  activateUnits: 2, // 【占位预填】激活费 ＝ 2 艘单位的派遣费

  /* ★★ M3d 迭代 3：**超出「出战上限（费率分界）」部分的加价费率**（用户口径：上限**不再拦截**，只加价）：
   *   · 设本次派遣 `n` 艘、**剩余正常名额** `r ＝ maxFleet − Σ 同时存活`：
   *       正常部分 `normal ＝ min(n, max(0, r))` ⇒ 每艘收 1× `deployCost`；
   *       超出部分 `m ＝ n − normal`（m ≥ 0）  ⇒ **一块**加价 ＝ `deployCost × (m + offset) ^ exponent`
   *         （★ **第一艘超出就已经加价**：m ＝ 1 ⇒ `(1 + offset) ^ exponent` 倍）；
   *       总价 ＝ 正常部分 ＋ 超出部分（★ 界面**合并求和、只给一个数**，不并列两段）；
   *   · **公式**只有一处实现：`systems/expedition.js` 的 `dispatchPriceOf`（本字段只提供**费率参数**）；
   *   · **取整**：逐资源键**向上取整**（`Math.ceil`；避免分毫数字，且加价不因取整而少收）；
   *   · 数值**【占位预填 · 待用户调校】**；缺省（字段缺失/非法）⇒ 引擎兜底常量 `DEFAULT_OVER_QUOTA`
   *     （`{ exponent: 1.5, offset: 1 }`，内置三档均已显式配置）。 */
  overQuota: { exponent: 1.5, offset: 1 }, // 【占位预填】超出部分总价 ＝ 单船费 × (m + 1) ^ 1.5

  /* ★ M3d：**难度描述词条键**（星门面板的"战区"行只读它 ⇒ 文案在 i18n、数值在配置，界面零硬编码） */
  descKey: 'starfield.h1.desc',

  /* ★ M3d：**结束演出（变白）每环 tick 数**（倒计时结束 ⇒ 以中心恒星为起点、按到中心的距离环逐环变白）：
   *   · 第 0 环＝中心恒星（`isStar`，坐标恒 `(0,0)`），第 n 环 = 「到中心四舍五入距离 == n」的全部星区；
   *   · **逐环间隔 = 本字段**（单位 tick；20tps ⇒ 20t ⇒ 1 秒/环）；演出总时长 = 环数 × 本字段；
   *   · 数值**【占位预填 · 待用户调校】**；缺省 ⇒ 引擎兜底 20（内置三档均已显式配置）。 */
  collapseRingTicks: 20, // 【占位预填】每环变白间隔（tick）
