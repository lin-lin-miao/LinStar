/* ===== data/modules/drone/laserTurretSpawn.js —— 激光炮塔（召唤 · 无人机类 · 固定炮塔） =====
 * 功能：召唤一个【激光炮塔】临时单位，自带**三个**与召唤模块**同等级**的模块：
 *   **两枚「激光」**`attack/laser`（不受槽限 ⇒ 同一单位双份输出）
 *   ＋「再生护盾」`shield/regenShield`（持续型：耗能回盾）。
 * 来源：`无人机模块设定.md`「激光炮塔：搭载2个激光模块，与一个再生护盾，能量恢复为30，能量上限1000，
 *   存在时间2400」——**energyRegen 30 / energyCap 1000 / lifespan_ticks 2400 为文档给定硬数值，
 *   逐字照填**（hp/护盾文档未给 ⇒ 占位）。
 * 图标：**文档要求“两个炮塔图标需要使用一个实心的正六边形SVG作为图标”** ⇒ 本模块 `icon` 指向
 *   `assets/img/激光炮塔.svg`（**素材由美术另行交付**；无人机类棕 `--cat-drone`）。
 ...
 *   - 携带模组 `modules`：**三条**（激光 ×2 + 再生护盾），等级＝召唤模块等级、force 安装不受槽限；
 ...
 *   · 【占位预填】`base.hp 40`、`base.shieldCap 20`（炮塔比无人机稍厚，**无文档依据、纯占位**）、
 *     `maxSummoned 1/2/3/4`（炮塔**存在时间长**、单体输出高 ⇒ 在场上限取得比无人机保守）、
 *     `cooldown_ticks 200` 与 `energy_cost 50`（照激光无人机 Lv1 占位，各级沿用 Lv1）；
 *   · 逐级表：**只放大占位项**（hp/shieldCap、在场上限），文档给定的能量两项与存在时间**各级不变**。
 * 数值说明：【占位预填 · 待用户调校】（文档给定的三项除外）。
 */
export default {
  id: 'laserTurretSpawn',
  nameKey: 'module.laserTurretSpawn', // i18n -> 激光炮塔（未显式命名时作为召唤单位显示名）
  name: '激光炮塔',
  icon: 'assets/img/激光炮塔.svg', // ★ 实心正六边形 SVG（美术另交付）；召唤单位共用本图标
  category: 'drone',
  ...
  effects: {
    ...
    summon: {
      type: 'drone', // 通用无人机模板
      // 携带：**两枚激光**（同一 moduleId 写两条 ⇒ 引擎逐条 force 安装成两个独立实例）+ 再生护盾
      //   （等级＝召唤模块等级，不受槽限 —— 既有口径，无需改动引擎）
      modules: [{ moduleId: 'laser' }, { moduleId: 'laser' }, { moduleId: 'regenShield' }],
      attrs: {
