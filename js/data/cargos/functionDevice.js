/* ===== data/cargos/functionDevice.js —— 功能设备（类型色＝功能类） =====
 * 体例与 `data/modules/<类别>/<模块>.js` **完全一致**（一物一文件，注册表见 `data/cargos/index.js`）。
 *  · `colorKey: '--cat-function'` ＝**功能类代表色**（`#7ee29a`）——与功能类模块芯片**共用同一变量**；
...
export default {
  id: 'functionDevice',
  nameKey: 'cargo.functionDevice', // i18n -> 功能设备 / Function Device
  name: '功能设备',
  type: 'functionDevice',
  colorKey: '--cat-function',      // ★ 类型色来源（CSS 变量名，不硬编码色值）
  tons: 5,
  loadTicks: 300,
  bonus: 1,
  maxLevel: 16,
