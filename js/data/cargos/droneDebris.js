/* ===== data/cargos/droneDebris.js —— 机骸碎片（类型色＝无人机类） =====
 * 体例与 `data/modules/<类别>/<模块>.js` **完全一致**（一物一文件，注册表见 `data/cargos/index.js`）。
 *  · `colorKey: '--cat-drone'` ＝**无人机类代表色**——本轮按用户口径改为**棕色**
 *    （`css/base.css :root` 的 `--cat-drone`，原 `#ffd1a8`）——与无人机类模块芯片**共用同一变量**；
...
export default {
  id: 'droneDebris',
  nameKey: 'cargo.droneDebris', // i18n -> 机骸碎片 / Drone Debris
  name: '机骸碎片',
  type: 'droneDebris',
  colorKey: '--cat-drone',      // ★ 类型色来源（CSS 变量名，不硬编码色值）
  tons: 5,
  loadTicks: 300,
  bonus: 1,
  maxLevel: 16,
